...
| RAG 负例拦截（6 条不该命中的 query） | 1/6 | **6/6** | Cross-Encoder 精排当闸门（`score_threshold=0.264`），正例保住 53/53（一格不丢）；**代价 +2.6s/次** |
...
6. **V6.0**：混合检索（BM25 + RRF）+ Rerank 精排闸门 + 修正 Embedding 池化（mean → CLS），检索指标全部重测（含三个被实测否掉的调优假设：同文档限流、向量阈值失配、融合前文档级去重）
...
- **四层防幻觉**：Prompt 强约束 → 知识增强 → 相似度阈值过滤（默认 0.514，不达标直接丢弃）→ 文档溯源
...
|---|---|---|---|
| 纯向量 | 46/53 | 52/53 | 0.921 |
| 纯 BM25 | 47/53 | 53/53 | 0.937 |
| **混合（RRF）** | **51/53** | **53/53** | **0.981** |
| 混合 + 精排重排 | 50/53 | 52/53 | 0.959 |

混合检索**三项指标全部第一**：Hit@1 与 MRR 明显领先，Hit@3 与纯 BM25 并列满分，**未命中归零**。
「混合 + 精排重排」一列是把精排分数用于**重排**（`mode="rerank"`），实测在本语料上为负收益，
故生产默认走 `mode="filter"`（RRF 排序 + 精排当闸门），该项仅为 A/B 对照保留。

精排分数当**准入闸门**时（`score_threshold=0.264`），负例拦截从 1/6 提升到 **6/6**，
**正例保住 53/53（一格不丢）**。参数见 `config.py` → `RAG_CONFIG`，实现见 `utils/hybrid_retriever.py` / `utils/reranker.py`。
...
- 如果模型目录缺 `modules.json` / `1_Pooling/config.json`，`sentence-transformers` 会**静默降级成 mean pooling**，只在 stderr 打一行 `Creating a new one with mean pooling` —— 没有任何报错，症状只是「检索变差」，极易长期潜伏。本项目就踩过：修正后纯向量 MRR 0.865→0.893、Hit@3 48→50
  > 注：这一组是**修正标注之前**测得的，与当前报告（纯向量 Hit@3 52/53、MRR 0.921）不同口径，不可混用。池化修正的收益本身未随标注变化而重测。
...
### RRF 的 chunk 数偏置（已定位 → 修复方案已证伪 → 已归因到标注）

**现象**：修正标注前，混合检索 Hit@3 只有 51/53，低于纯 BM25 的 53/53；失败的两条是
「国际长途航班可以订什么舱位」和「紧急出差可以后补审批吗」。

**假设**：RRF 按 **chunk 累加**打分（`RRF(d) = Σ w/(k+rank)`），一篇文档若有多个 chunk 被召回，
就会各自贡献一份分数 —— 等于给「chunk 多的文档」变相加权，语料里体积最小的文档被系统性压低。

**验证**（`python scripts/diag_rrf_bias.py`，只读诊断，不改生产配置）：

- 语料 chunk 数分布不均：90 chunk / 12 篇，单篇 **5~10** 个，**投票机会相差 2.00 倍**；
  两条失败用例的期望文档 chunk 数**恰好都是最小值 5**
- 逐票分解是决定性证据：
  - 「国际长途航班可以订什么舱位」的期望文档是 **BM25 第 1 名（10.32，领先第 2 名 7.55 一大截）**，
    却只有 **2 票**（RRF 0.03888），融合后排 **#5**；赢家 `03_booking_guide.txt` 在任何一路都没进过前 2，
    靠 **4 票**拿到 0.07770 —— **两倍于期望文档**
  - 「紧急出差可以后补审批吗」的期望文档只有 **1 票**（0.02419），`04_faq.txt` 靠 **11 票**拿到
    0.22065 —— **9 倍**。该 query 的 BM25 前 10 里有 **7 条**都是 `04_faq.txt`
- **结论：偏置机制确认成立**

**修复方案被证伪**。在 RRF 融合前对每条通道按文档去重（每篇只以最佳 chunk 投票），实测**显著变差**：

> 下表四行均在**修正前的标注**上测得（与本节「现状」一致），故基线是 50/53 而非修正后的 51/53。

| 方案 | Hit@1 | Hit@3 | MRR |
|---|---|---|---|
| 现状（生产配置） | **50/53** | **51/53** | **0.9528** |
| 融合前完全去重（每文档 1 票） | 43/53 | 50/53 | 0.8679 |
| 每文档最多 2 票 | 50/53 | 51/53 | 0.9528 |
| 每文档最多 3 票 | 50/53 | 51/53 | 0.9528 |

完全去重**弄坏 7 条、只修好 1 条**（配对符号检验 **p = 0.016，显著变差**）。
**根因**：多 chunk 命中**既是偏置、也是真实信号** —— 一篇文档有 4 个 chunk 都相关，本来就说明它更相关；
硬去重把信号和偏置一起扔掉了。

限流到 2/3 票则 **53 条中 Hit@1 无一条翻转、Hit@3 也不变**：`03_booking_guide`
（每通道恰好 2 票）不受影响；`04_faq` 虽从 11 票被削到 4 票，仍稳稳领先；
而期望文档自己本来就只有 1~2 票，**没有被削减的空间** —— 等比削减改变不了相对次序。
**该方案不要实施。**

**真正的根因是标注过窄** —— 两条「失败」的期望文档都只是**多个正确答案之一**：

| query | 答案实际出现在 |
|---|---|
| 紧急出差可以后补审批吗 | `12_seasonal_policies.txt` 行115 · `04_faq.txt` 行6 · `01_travel_standards.txt` 行19 —— **三篇都直接回答** |
| 国际长途航班可以订什么舱位 | `01_travel_standards.txt` 行31 与 `10_international_travel.txt` 行46 写的是**同一条规则** |

`eval_retrieval.py` 的 docstring 本就规定「两篇都答得上」要用**元组标注**，这两条没按规则标。
修正后混合检索 Hit@3 达到 **53/53**、未命中归零，**原「未解问题」的前提不成立**。

> ⚠️ **方法论提醒**：修正标注**同时抬高了全部四路的分数**，新旧报告的绝对数字不可直接混用。
> 扩标注集到 100+ 条前，**务必先统一元组标注规范**，否则噪声会随规模放大。
> 更彻底的做法是把 query 写得更具体、使其唯一指向一篇（例如 `12` 行115 的
> 「需在出发前电话或消息告知直属主管」是它独有的内容），这比放宽标签更严谨。
