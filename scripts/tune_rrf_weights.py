...
背景（实测）：
    纯 BM25        Hit@1=47/53  Hit@3=53/53  MRR=0.937
    混合(RRF)      Hit@1=51/53  Hit@3=53/53  MRR=0.981
  混合检索三项指标均已不劣于纯 BM25，Hit@3 并列满分。

  ⚠ 早期记录曾写「单靠 BM25 的 Hit@3 是满分，等权混合反而掉 2 条，说明关键词路
  排序更准、被向量路稀释」—— 该结论**已作废**：那 2 条是**标注过窄**造成的假象
  （期望文档只是多个正确答案之一），修正标注后混合同样达到 53/53。
  详见 README「RRF 的 chunk 数偏置」。

  因此本脚本现在的意义变了：Hit@3 已经饱和，扫描权重**只影响 Hit@1 / MRR**
  （即排名质量），不会再改变召回完整性。现行 1:1.5 是在修正前的标注上扫出来的，
  最优点可能已移动，值得重扫一次。
