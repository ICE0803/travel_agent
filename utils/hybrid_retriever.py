...
def limit_per_doc(
    ...
    为什么需要（实测）：
      ...
      3 条**全是** `04_faq.txt`（该 query 的 BM25 前 10 里有 7 条都是 `04_faq.txt`），
      重复内容进 prompt 既稀释上下文、又白白烧 token。
    ...
    ⚠ 实测结论（2026-09-14，诊断脚本 `scripts/diag_rrf_bias.py`，详见 README
      「RRF 的 chunk 数偏置」）：

    1. 本函数是**输出侧**去重，对混合检索与精排闸门的 Hit@k / MRR **完全中性**
       （只把重复槽位换成排名更靠后的其他文档，期望文档名次不变），
       但会拉低 `mode="rerank"` 一列。故生产默认 `max_per_doc=0`。

    2. 曾经设想的"在 RRF 之前先按文档去重各路召回"（让每篇文档只以最佳 chunk
       投票，从源头消除文档级加权）**已实测证伪**：Hit@1 50→43、
       MRR 0.9528→0.8679，弄坏 7 条只修好 1 条（配对符号检验 **p = 0.016，显著变差**）。
       根因：多 chunk 命中**既是偏置、也是真实信号** —— 一篇文档有 4 个 chunk
       都相关，本来就说明它更相关；硬去重把信号和偏置一起扔掉了。**不要实施。**

    3. 早期把"混合检索 Hit@3=51/53 低于纯 BM25 的 53/53"归因到本机制，
       **该归因已作废**：那两条用例是**标注过窄**（期望文档只是多个正确答案之一），
       按元组标注修正后混合同样达到 **53/53**。偏置机制本身成立 ——
       语料 90 chunk / 12 篇、单篇 5~10 个、投票机会相差 **2.00 倍**，
       逐票分解可复现（期望文档 BM25 #1 却因只有 2 票而排到融合 #5）——
       但它并不解释那两条"失败"。
    """
    if not items:
        return []
